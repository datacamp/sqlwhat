...
@pytest.mark.parametrize('query, path, target_desc', [
    (
    ...
    (
        'SELECT a FROM b',
        [[check_node, ['SelectStmt']], [check_field, ['from_clause']]],
        '`FROM` clause of the `SELECT` statement'
    ),
    (
        'SELECT a FROM b',
        [[check_node, ['SelectStmt']], [check_field, ['target_list', 0]]],
        'first entry in the target list of the `SELECT` statement',
    ),
    (
        'SELECT a FROM b INNER JOIN c HAVING(d)',
        [[check_node, ['SelectStmt']], [check_field, ['from_clause']], [check_field, ['left']]],
        None # Doesn't generate anything for now!
    ),
    (
        'SELECT a FROM b INNER JOIN c HAVING(d)',
        [[check_node, ['SelectStmt']], [check_node, ['JoinExpr']], [check_field, ['left']]],
    ...
    (
        ...
        [[check_node, ['SelectStmt']], [check_field, ['where_clause']], [check_field, ['left']]],
        None # Doesn't generate anything!
    ),
    (
        'SELECT a FROM b WHERE c BETWEEN 1 AND 2',
        [[check_node, ['SelectStmt']], [check_node, ['BinaryExpr']], [check_field, ['left']]],
...
# check_node - check_field -----------------------------------------------------------------

@pytest.mark.parametrize('stu, path, msg', [
    (
        '', 
        [[check_node, ['SelectStmt']], [check_node, ['JoinExpr']], [check_field, ['left']], [has_equal_ast, []]],
    ...
    (
        # using check_field('from_clause') instead of check_node('JoinExpr')
        'SELECT a FROM c INNER JOIN b HAVING(d)',
        [[check_node, ['SelectStmt']], [check_field, ['from_clause']], [check_field, ['left']], [has_equal_ast, []]],
...
@pytest.mark.parametrize('stu, path, msg', [
    ...
    (
        'SELECT a FROM b ORDER BY b', 
        [[check_node, ['SelectStmt']], [check_field, ['order_by_clause']], [check_node, ['SortBy']], [has_equal_ast, []]],
